...
export default function Preferiti() {
  const { wishlist, toggleWishlist, syncCatalog } = useStore();
  const { products, ready } = useProducts();

  useEffect(() => {
    if (ready) syncCatalog(products);
  }, [ready, products, syncCatalog]);
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] font-sans">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 pt-8 pb-20">
        <Link to="/" className="inline-flex items-center gap-2 text-sm text-[#6e6e73] hover:text-[#0071E3] transition-colors mb-4">
        ...
        {wishlist.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl">
            ...
            <Link to="/catalogo" className="inline-block px-6 py-3 bg-[#1d1d1f] text-white text-sm font-semibold rounded-full hover:bg-[#0071E3] transition-colors">
              Vai al Catalogo
            </Link>
          </div>
        ) : (
            ...
            {wishlist.map(item => (
              <div key={item.id} className="group bg-white rounded-2xl overflow-hidden flex flex-col">
                <Link to={`/scheda-prodotto?id=${item.id}`} className="block relative overflow-hidden" style={{ aspectRatio: '1 / 1' }}>
                  <Image src={item.image} alt={item.name} className="h-full w-full p-3 transition-transform duration-500 group-hover:scale-[1.025]" fittingType="fit" />
                </Link>
                <div className="p-3 flex flex-col flex-1">
                  <Link to={`/scheda-prodotto?id=${item.id}`} className="text-sm font-semibold text-[#1d1d1f] line-clamp-2 mb-1 hover:text-[#0071E3]">{item.name}</Link>
