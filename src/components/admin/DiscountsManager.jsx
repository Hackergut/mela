...
export default function DiscountsManager({ password }) {
  ...
  const save = async (e) => {
    ...
    const payload = { ...editing, value: Number(editing.value) || 0, max_uses: editing.max_uses ? Number(editing.max_uses) : null, usage_count: editing.usage_count || 0 };
              ...
              <div key={d.id} className={`bg-white rounded-2xl p-5 border border-gray-100 ${bulk.selected[d.id] ? 'ring-2 ring-[#FF6B35]' : ''}`}>
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => setEditing(d)} className="w-8 h-8 rounded-lg bg-gray-100 hover:bg-gray-200 flex items-center justify-center"><Pencil size={14} /></button>
                ...
                <p className="text-2xl font-bold text-[#FF6B35]">{d.type === 'percent' ? `−${d.value}%` : `−€${(d.value / 100).toFixed(2)}`}</p>
          ...
          <form onSubmit={save} className="bg-white rounded-3xl w-full max-w-md p-6 my-8 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="block">
                ...
                <select value={editing.type} onChange={e => setEditing({ ...editing, type: e.target.value })} className={INP}>
              ...
              <label className="block">
                ...
                <input required type="number" value={editing.value} onChange={e => setEditing({ ...editing, value: e.target.value })} className={INP} />
              </label>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <label className="block">
            ...
            <div className="flex gap-3 pt-2">
              <button type="submit" className="flex-1 py-3 bg-[#FF6B35] text-white text-sm font-semibold rounded-xl">Salva Codice</button>
