...
export default function ShopifyManager({ password }) {
  ...
  const saveCreds = async () => {
    ...
    try {
      await base44.functions.invoke('shopify-sync', { password, operation: 'save_creds', payload: { shop_domain: shopDomain, access_token: accessToken } });
      setAccessToken(''); setHasToken(true); setConfigured(true);
      ...
      <div className="bg-white rounded-2xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#6e6e73]">Sincronizza ordini, clienti e vendite storiche dal tuo negozio Shopify</p>
          </div>
          {configured && <span className="ml-auto px-2.5 py-1 text-[10px] font-bold rounded-full bg-green-100 text-green-700 flex items-center gap-1"><CheckCircle2 size={12} /> Connesso</span>}
        </div>
